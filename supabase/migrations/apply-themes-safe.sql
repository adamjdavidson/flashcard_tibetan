-- Safe Themes Migration (No DROP statements)
-- This version is safe to run - it only creates what doesn't exist
-- Use this in Supabase SQL Editor if you get "destructive operation" warnings
...
-- Enable RLS (idempotent - can run multiple times)
ALTER TABLE themes ENABLE ROW LEVEL SECURITY;
ALTER TABLE user_theme_preferences ENABLE ROW LEVEL SECURITY;

-- RLS Policies (using DO $$ blocks to handle if exists)
DO $$
BEGIN
  ...
  IF NOT EXISTS (
    ...
    AND policyname = 'Anyone can read themes'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' 
    AND tablename = 'user_theme_preferences' 
...
-- Create default theme from current colors (ON CONFLICT handles duplicates)
...
-- Triggers (using DO $$ blocks to handle if exists)
DO $$
BEGIN
  ...
  -- Trigger for user_theme_preferences table
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger 
    WHERE tgname = 'update_user_theme_preferences_updated_at'
  ) THEN
